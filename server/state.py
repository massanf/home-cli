...
def get_is_home() -> bool:
    return load_state().get("is_home", True)


def set_is_home(value: bool):
    state = load_state()
    state["is_home"] = value
    save_state(state)


def update_snapshot_device(category, device, value):
    snapshot = load_snapshot()
    if category not in snapshot:
        snapshot[category] = {}
    snapshot[category][device] = value
